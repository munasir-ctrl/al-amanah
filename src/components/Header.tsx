...
export function Header() {
  ...
  return (
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 h-32 md:h-40 flex items-center ${
          scrolled
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex items-center justify-between">
          {/* Logo - Perfectly Centered & Aligned */}
          <Link to="/" className="flex items-center justify-center h-full my-auto overflow-visible">
            <img
              src="/logo.png"
              alt={siteConfig.name || "Logo"}
              className="h-24 sm:h-32 md:h-36 w-auto object-contain max-w-[380px] sm:max-w-[500px]"
            />
          </Link>
          ...
          <div className="absolute right-0 top-0 bottom-0 w-[85%] max-w-sm bg-white shadow-premium animate-slide-in flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-navy-100 h-32">
              <Link to="/" onClick={() => setMobileOpen(false)} className="flex items-center justify-center h-full">
                <img src="/logo.png" alt="Logo" className="h-24 object-contain w-auto max-w-[300px]" />
              </Link>
